  ...
  return (
    <div className="flex gap-2 overflow-x-scroll md:overflow-x-hidden snap-x snap-mandatory px-2 md:mx-0 md:px-0 overflow-visible">
      {blogs.map((blog) => (
        <div
          key={blog.blog_id}
          className="flex flex-col items-start gap-2 sm:gap-4 w-[60vw] sm:w-[55vw] md:w-[17vw] shrink-0 snap-start rounded-3xl p-4"
        >
          <Image
            src={blog.blog_img}
            width={250}
          ...
          />
          <h1 className="text-lg font-semibold line-clamp-3 md:line-clamp-2">
            {blog.blog_title}
          </h1>
          <p className="line-clamp-1 md:line-clamp-3 text-gray-600 text-sm">
            {blog.short_desc}
          </p>
